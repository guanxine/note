...
主线程提交任务到线程池，主线程不关心任务被哪个线程执行。

## 两阶段终止模式
第一个阶段主要是 T1 向线程 T2 发送终止指令，而第二阶段则是 T2 响应终止指令

Java 里终止指令
Java 线程进入终止状态的前提是线程进入 RUNNABLE 状态，而实际上线程也可能处于休眠状态。
要终止一个线程，首先要把线程状态从休眠状态转换到 RUNNABLE 状态。
靠 Thread.interrupt() 方法。他可以将休眠状态的线程转化到 RUNABLE 状态。
RUNABLE 如何终止呢，优雅方式是 JAVA 线程自己执行完 run 方法。
所以我们一般采用的方法是设置一个标志位。线程检查这个标志位，发现符合终止条件，则自动退出 run() 方法。
...
## 生产者-消费者: 流水线思想
Java 线程池本质就是生产者-消费者模式实现的，所以每当使用线程池时，其实就是在应用生产者-消费者模式

核心: 一个任务队列(主要作用：平衡生产者和消费者)
生产者线程生产任务，并将任务添加到任务队列，消费者线程从任务队列中获取任务并执行。
重要的优点
1. 解耦(架构)， 解耦的关键：组件之间的依赖关系和通信方式必须受限。
-- 生产者消费者没有任何依赖关系，它们彼此之间的通信只能通过任务队列
2. 支持异步，能够平衡生产者和消费者的速度差异。

平衡生产者和消费者的速度差异，例如，生产者：消费者=1:3, 生产者3个线程，消费者1个线程。

场景(线程池中的线程每次只能从任务队里中消费一个线程来执行，大部分并发策略没问题。但有些场景还需要自己来实现)
1. 批量执行任务。
2. 支持分阶段提交
  ++ 日志组件异步刷盘操作的本质其实就是一种分阶段提交

分布式
分布式场景下，可以借助分布式消息队列(MQ) 来实现生产者-消费者模式。
MQ 一般会支持两种消息类型，一种点对点，一种发布订阅模式。



## 案例1 Guava RateLimiter 


## 案例2 Netty
BIO: 阻塞I/O, 所有 read 和 write 操作都会阻塞当前线程。所以，使用 bio 模型，一般都会为每个 socket 分配一个独立的线程
这样不会因为阻塞当前线程而影响对其他 socket 的读写。为了避免频繁创建，消耗线程，可以采用线程池。
BIO 模型无法解决百万链接的问题。
线程的大部分时间都在等待 I/O 就绪。在 I/O 就绪前调用线程会一直阻塞，也就无法处理其他socket链接了。

优化：提高线程的利用率，用一个线程来处理多个连接。

Java 提供非阻塞的 API, 利用非阻塞 API 就能够实现一个线程处理多个链接了。
### Reator 模式


### Netty 
核心：事件循环(EventLoop)
Netty: bossGroup 处理连接请求，workerGroup 处理读写请求(有多个 EventLoop -- 负载均衡算法 -- 轮询算法)。

每个网络连接都关联到一个线程上，好处: 对于一个网络连接，读写操作都是单线程执行，从而避免了并发程序的各种问题。
...
性能不是很高，因为 Java 中阻塞队列是用锁实现的。高并发下锁的征用对性能影响很大。
